...
if(class_exists('Aelia\EDD\Aelia_Install')) {
	return;
}
...
class Install {
	...
	// TODO Use Aelia\EDD\Messages class to handle messages
		...
		foreach($this->messages as $message) {
			...
			if($message->level != $last_message_level) {
				...
				if(!empty($message_header)) {
					echo '<h4 class="edd_aelia message_header">';
					echo $message_header;
					echo '</h4>';
				}
			}
			...
			echo '<p class="edd_aelia message">';
			echo $output_msg;
			echo '</p>';
	...
	public function update($plugin_id, $new_version) {
		...
		$this->add_message(E_USER_NOTICE,
											 sprintf(__('Running updates for plugin <span class="edd_aeliaplugin_id">%s</span>...'),$plugin_id));
		...
		if($result === true) {
			...
			$this->add_message(E_USER_NOTICE, __('<span class="edd_aeliaimportant">Update completed successfully</span>.'));
		}
		else {
			$this->add_message(E_USER_ERROR, __('<span class="edd_aeliaimportant">Update halted</span>. Please review displayed messages and ' .
